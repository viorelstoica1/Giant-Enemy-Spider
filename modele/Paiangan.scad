...
module roata_transmisie(diametru=30, diametru_tub=9.6, gauri=2, grosime=5, diametru_gauri = 3, diametru_cerc_gauri = 25,lungime_tub=10, latime_capat = 6.8, inaltime_capat = 10){
    difference(){
...
module structura_dreptunghi(lungime=latime_core-offset_roata_zimtata*2-diametru_roata_zimtata+grosime_core, diametru=12, diametru_gaura=7, grosime = grosime_core, inaltime=inaltime_core/2+grosime_core/2, diametru_surub=3){
    $fn=60;
    color("green")
    translate([-lungime/2,0,0])difference(){
        union(){
            ...
            translate([grosime/2,grosime,grosime/2]) rotate([90,0,0]) cylinder(grosime,diametru/2, diametru/2);
            translate([lungime-grosime/2,grosime,grosime/2]) rotate([90,0,0]) cylinder(grosime,diametru/2, diametru/2);
            //suport suruburi
        ...
        translate([grosime/2,grosime+0.01,grosime/2]) rotate([90,0,0])cylinder(grosime+0.02,diametru_gaura/2, diametru_gaura/2);
        translate([lungime-grosime/2,grosime+0.01,grosime/2]) rotate([90,0,0])cylinder(grosime+0.02,diametru_gaura/2, diametru_gaura/2);
    }
}
module structura_triunghi(lungime=lungime_core/3, diametru=15, diametru_gaura=10, grosime = grosime_core, inaltime=inaltime_core/2, diametru_surub=3){
    ...
    difference(){
        union(){
            ...
            translate([0,grosime,0]) rotate([90,0,0])cylinder(grosime,diametru/2, diametru/2);
            translate([lungime,grosime,0]) rotate([90,0,0])cylinder(grosime,diametru/2, diametru/2);
        }
        ...
        translate([0,grosime+0.01,]) rotate([90,0,0])cylinder(grosime+0.02,diametru_gaura/2, diametru_gaura/2);
        translate([lungime,grosime+0.01,0]) rotate([90,0,0])cylinder(grosime+0.02,diametru_gaura/2, diametru_gaura/2);
    }
}
...
module ansamblu(){
    for(j=[0:3]){
        translate([j*lungime_core/3,0,0]) rotate([0,$t * 360,0]) rotate([90,0,0])roata_zimtata();
        translate([j*lungime_core/3-diametru_roata_zimtata/2-offset_roata_zimtata, -diametru_roata_zimtata/2 - offset_roata_zimtata,0]) rotate([$t * 360,0,0]) rotate([0,90,0]) roata_zimtata();
        //translate([j*lungime_core/3,40,0]) rotate([0,$t * 360,0]) rotate([90,0,0]) roata_transmisie();
    }
    /*for(j=[0:2]){
...
if (1){
    ...
    //translate([-118.75,-25,32.75]) adaptor_pcb();
}
...
module adaptor_pcb(grosime=7.5, inaltime=2, diametru_gauri=3, lungime = 107.5, latime = 50, diametru_gauri_pcb=3, lungime_pcb=30, latime_pcb=65, inaltime_adaptor_pcb=10, lungime_hbridge=60, latime_hbridge=55, inaltime_baterie=30, latime_baterie=15, lungime_baterie = 40, grosime_baterie=4){
    color("Green"){
        difference(){
            union(){
...
module adaptor_camera(grosime = 3, diameru_surub = 3){
    difference(){
        union(){
            translate([15,0,0])cube([30,8,grosime]);
            translate([0,8,0])cube([60,5,21]);
        }
        translate([20,4,-0.01]) cylinder($fn = 30, 10, diameru_surub/2, diameru_surub/2);
        translate([40,4,-0.01]) cylinder($fn = 30, 10, diameru_surub/2, diameru_surub/2);
        translate([5, 8-0.01,grosime]) cube([47.5,5+0.02,20]);
        translate([2, 9.75,grosime]) cube([5,1.5,20]);
        translate([52.5-0.01, 8-0.01,grosime]) cube([5.2,3+0.02,20]);
        translate([52.5-0.01, 10.5,grosime]) cube([5,3,7]);
        translate([54.5, 13.5,grosime+9.5]) rotate([90,0,0]) cylinder($fn = 30, 5,1,1);
    }

}
//adaptor_pcb();
//translate([100,20,0]) rotate([0,0,-90])adaptor_camera();